...
2. 已有 Figma 页面先锁定最终布局和项目视觉，再映射 UDesign 单组件或组合；非核心缺失才使用云随内部补全策略。
...
| 二级功能页头 | `Space` + `Button` + `Typography` + `Tag` | 返回、20px 标题、AI 免责声明 | 组合名 `YunsuiFeatureHeader`，结构遵循 Figma；非核心密度细节才参考云随内部补全策略 |
...
| 纯图标操作 | `Button` + `Icon` | 方形图标按钮或文字按钮；图标来自 [icon-library.md](icon-library.md)；提供 `aria-label/title` |
...
| 空/无记录/不可用 | `Empty` | `type`, `description`, `action`; 正式 Figma 插图优先，无正式插图时按 [illustration-rules.md](illustration-rules.md) 使用 UDesign Empty 兜底 |
