...
- 云随内部补全策略只处理正式 Figma、云随成熟模式和 UDesign 均未定义的非核心细节。
...
- 基础功能图标必须使用本地 UDesign / TDesign Icons 官方 SVG，详见 [icon-library.md](icon-library.md)。
- 不用 emoji、临时字符、AI 手写 SVG 或第三方图标库模拟关键操作图标。
- 常规图标 16px，强提示 20px 或 24px；图标与文字间距 4px 或 6px。
- 图标默认使用 `currentColor`，由父级文字色或 UDesign Token 控制。
- 空状态/失败页必须提供原因和下一步；若无正式 Figma 插图，按 [illustration-rules.md](illustration-rules.md) 使用 UDesign Empty 或云随中性占位。
- 普通空状态缺少 Figma 插图不阻断 Demo 生成；只有插图承载核心业务含义或强品牌主视觉时，才列为设计缺口。
- 禁止自行生成卡通机器人、笑脸、装饰徽章或随机插画。
...
- 图标使用 `assets/icons/tdesign-icons/svg/` 中的官方 SVG，并只内联本次实际用到的图标；不得依赖会过期的 Figma 临时资源地址。
- 空状态插图可使用 `assets/illustrations/udesign-empty/` 中的官方状态 SVG；不得把兜底插图伪装成正式项目插图。
